      ...
      <div className="container mx-auto px-4 md:px-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-12 md:gap-16 mb-16 md:mb-24">
          <div className="space-y-6 md:space-y-8">
            ...
            <div className="flex gap-4">
              {[
                { n: 'fb', l: 'https://facebook.com/get5starsreview' },
                { n: 'tw', l: 'https://twitter.com/get5starsreview' },
                { n: 'ig', l: 'https://instagram.com/get5starsreview' },
                { n: 'li', l: 'https://linkedin.com/company/get5starsreview' }
              ].map((s, i) => (
                <a key={i} href={s.l} target="_blank" rel="noopener noreferrer" className="w-9 h-9 md:w-10 md:h-10 rounded-full bg-white/5 flex items-center justify-center text-white hover:bg-[#16A34A] transition-all cursor-pointer" aria-label={`Follow us on ${s.n}`}>
                  <span className="font-bold text-[10px] md:text-xs uppercase">{s.n}</span>
                </a>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-3 md:space-y-4 text-xs md:text-sm font-medium text-slate-400">
              <li><button onClick={() => onScrollToSection?.('services')} className="hover:text-[#16A34A] transition-colors text-left">GBP Management Service</button></li>
              <li><button onClick={onAgencySignup} className="hover:text-[#16A34A] transition-colors text-left">Agency Partner Program</button></li>
              <li><button onClick={() => onScrollToSection?.('how-it-works')} className="hover:text-[#16A34A] transition-colors text-left">Local Citation Building</button></li>
              <li><button onClick={onToolsClick} className="hover:text-[#16A34A] transition-colors text-left">AI Review Automation</button></li>
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-3 md:space-y-4 text-xs md:text-sm font-medium text-slate-400">
              <li><button onClick={onHomeClick} className="hover:text-[#16A34A] transition-colors text-left font-bold text-white">Home</button></li>
              <li><button onClick={onAboutClick} className="hover:text-[#16A34A] transition-colors text-left">About Us</button></li>
              <li><button onClick={onBlogClick} className="hover:text-[#16A34A] transition-colors text-left">Latest Blog Posts</button></li>
        ...
        <div className="pt-8 md:pt-12 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-6">
           <div className="flex flex-wrap justify-center md:justify-start gap-4 md:gap-8 text-[9px] md:text-[11px] font-bold uppercase tracking-widest">
             <span className="text-white">© 2025 Get5StarsReview</span>
