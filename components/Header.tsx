        ...
        <div className="container mx-auto px-6">
          <div className={`glass-panel rounded-[32px] px-8 py-3 flex items-center justify-between transition-all duration-500 ${isScrolled ? 'shadow-2xl border-slate-200' : 'border-transparent bg-transparent shadow-none'}`}>
            ...
            <div className="flex items-center gap-12">
              <div className="cursor-pointer hover:scale-105 transition-transform" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
            ...
            <div className="flex items-center gap-4">
              <button onClick={onLogin} className="hidden md:block text-[11px] font-black text-slate-900 uppercase tracking-widest hover:text-green-600 transition-colors">Login</button>
              <button 
      ...
      <div className={`fixed inset-0 z-40 bg-white lg:hidden transition-all duration-500 ${isMobileMenuOpen ? 'translate-y-0' : '-translate-y-full'}`}>
        <div className="pt-32 px-10 space-y-8">
           ...
           <div className="pt-10 space-y-4">
             <button onClick={onLogin} className="w-full py-5 bg-slate-100 rounded-2xl font-black uppercase tracking-widest">Login</button>
             <button onClick={onBusinessSignup} className="w-full py-5 bg-green-600 text-white rounded-2xl font-black uppercase tracking-widest shadow-xl">Get Started</button>
           </div>
        </div>
      </div>
    </>
