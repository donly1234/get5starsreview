...
const Header: React.FC<HeaderProps> = ({ 
  ...
  const navLinks = [
    ...
    { label: 'How It Works', action: () => onScrollToSection?.('how-it-works') },
      ...
      >
        <div className="h-full flex flex-col p-8 pt-24 overflow-y-auto">
           <div className="flex flex-col gap-6">
              {['AI Tools', 'How It Works', 'Services', 'Pricing', 'About', 'Blog'].map((item, idx) => (
                <button 
                 key={item} 
                 onClick={() => {
                   setIsMobileMenuOpen(false);
                   if (item === 'AI Tools') onToolsClick();
                   if (item === 'How It Works') onScrollToSection?.('how-it-works');
           ...
           {/* Fixed: Wrapped conditional class strings in quotes to resolve 'Cannot find name opacity' and JSX tree break errors */}
