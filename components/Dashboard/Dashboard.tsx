...
const TrialChecklist: React.FC = () => {
  const [completed, setCompleted] = useState<number[]>(() => {
    const saved = localStorage.getItem('g5sr_checklist');
    return saved ? JSON.parse(saved) : [1];
  });

  useEffect(() => {
    localStorage.setItem('g5sr_checklist', JSON.stringify(completed));
  }, [completed]);
  ...
  return (
    <div className="bg-white rounded-[32px] border border-slate-200 shadow-sm p-8 animate-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-10">
        <div>
          <h3 className="text-xl font-black text-slate-900">Trial Onboarding</h3>
          <p className="text-slate-500 text-sm mt-1">Get your account 100% optimized for ranking.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {items.map((item) => {
          ...
          return (
            ...
            >
              {isDone && (
                <div className="absolute top-4 right-4 w-6 h-6 bg-emerald-500 text-white rounded-full flex items-center justify-center shadow-lg animate-in zoom-in">
