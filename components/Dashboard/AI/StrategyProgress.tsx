...
const StrategyProgress: React.FC<StrategyProgressProps> = ({ onLaunchStrategy }) => {
  ...
  if (!activeStrategy) {
    return (
      <div className="bg-[#0F172A] rounded-[40px] p-10 text-white shadow-2xl border-b-4 border-emerald-500/30 flex flex-col md:flex-row items-center gap-10">
         <div className="w-20 h-20 bg-emerald-600/20 rounded-[28px] flex items-center justify-center text-4xl shadow-inner border border-emerald-500/20 shrink-0">🧠</div>
         <div className="flex-1 text-center md:text-left space-y-2">
            <h3 className="text-2xl font-black uppercase italic tracking-tighter leading-none">No Active AI Roadmap</h3>
            <p className="text-slate-400 font-medium">Generate a custom 30-day strategy to optimize your Google Maps ranking based on your industry trends.</p>
         </div>
         <button 
          onClick={onLaunchStrategy}
          className="px-8 py-4 bg-emerald-600 text-white rounded-2xl font-black uppercase tracking-widest text-[11px] shadow-lg shadow-emerald-600/20 hover:bg-white hover:text-black transition-all active:scale-95"
         >
           Get AI Blueprint
         </button>
      </div>
